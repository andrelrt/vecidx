...
struct smart_step
{
    smart_step( const std::vector< T2 >& ref ) : ref_( ref ){}
    ...
    typename std::vector< T2 >::const_iterator find( const T2& t2 )
    {
        ...
        size_t i = (mask == 0xffffffff) ? 8 : _bit_scan_reverse( mask + 1 ) >> 2;
...
int main(int /*argc*/, char* /*argv*/[])
{
    ...
    while( 1 ) 
    {
        ...
        size_t late = bench<smart_step, uint32_t>( "smart_step,  uint32", 0x000fffff, 100 );
